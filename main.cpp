...
int main(int argc, char *argv[]) 
{
    ...
    QCommandLineOption optionsOption(
        {"o", "options"},
            "Options for the binary given by -b (default '').",
            "opt_string");
	...
	int i = 0;
    while (netList.count() != 2) {
		netList << (std::string("Gopher ") + std::to_string(++i)).c_str();
    }
	
	i = 0;
	QStringList tmpBinList = parser.values(binaryOption);
	QStringList binList;

	// Debug fast testing
	//binList << "C://Users/C-60/Desktop/GopherVs/test/Pow";
	//binList << "C://Users/C-60/Desktop/GopherVs/Gopher";
	tmpBinList << "Uct";
	tmpBinList << "Pow";

	// Quick add the default path to the beginning of bin
	// to avoid typing it every time
    while (binList.count() != 2) {
		std::stringstream ss;
		ss << "C://Users/C-60/Desktop/GopherVs/";
		ss << tmpBinList[i].toStdString();
		binList << ss.str().c_str();
		++i;
    }
