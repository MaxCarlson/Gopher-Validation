...
bool Game::gtpClearBoard()
{
	if (!sendGtpCommand("clear_board"))
		return false;
	return true;
}
...
bool Game::gameStart(const VersionTuple &min_version) {
	//system(m_cmdLine.toStdString().c_str());
	// For looking at output to debug why it's not running
	//QProcess::execute(m_cmdLine);
	start(m_cmdLine);
	if (!waitForStarted()) {
...
bool Game::readMove() {
    ...
    if (m_moveDone.compare(QStringLiteral("pass"),
        ...
        m_passes++;
    } else if (moves.at(0).compare(QStringLiteral("resign"),
                                 Qt::CaseInsensitive) == 0) {
        m_resignation = true;
