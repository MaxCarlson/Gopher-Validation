	...
	do {
		...
		do {
			...
			else
			{
				...
				second.readMove();
				if (second.checkGameEnd())
					break;

				first.setMove(bmove + second.getMove());
				...
				second.setMove(wmove + first.getMove());

				first.nextMove();
				nextMove = second.nextMove();
			}

        //} while (first.nextMove() && m_state.load() == RUNNING);
		} while (nextMove && m_state.load() == RUNNING);
        ...
        if (m_state.load() == RUNNING) {
			...
			QTextStream(stdout) << "Clearing engine boards." << endl;
			first.gtpClearBoard();
			second.gtpClearBoard();
			// Reset the game if we're not rebooting engine each time
			first.resetGame();
			second.resetGame();
			++gameNum;
